...
describe('getUserHandle', () => {
  ...
  it('returns publicAddress from the first wallet when primaryZID is absent', () => {
    const user = { primaryZID: null, wallets: [{ id: 'wallet-id-1', publicAddress: 'address456' }] };
    expect(getUserHandle(user.primaryZID, user.wallets)).toEqual('address456');
  });
